...
subsetsens['Significant'] = (subsetsens['pval'] < 0.01) & ((subsetsens['fold_change'] > 1) | (subsetsens['fold_change'] < -0.99))
...
texts = subsetsens_sub.sort_values(by=['logPValue'],ascending=[True]).reset_index(drop=True).head(5).append(subsetsens_sub.loc[subsetsens_sub['Name']=='HIGD1A'],ignore_index=True)
...
plt.savefig('PC3_TMM_Mitochondria_xlim5_042723_goCCc.png',dpi=600,bbox_inches ='tight',pad_inches=1,transparent=True)
plt.show()
